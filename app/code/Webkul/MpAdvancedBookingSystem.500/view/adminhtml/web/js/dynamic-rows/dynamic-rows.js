        ...
        return uiCollection.extend(
            {
                ...
                initialize: function () {
                    ...
                    $('body').on(
                        'change',
                        'input[name="product[is_multiple_tickets]"]',
                        function () {
                            if ($(this).prop('checked')) {
                                $('#wk-bk-add-new-ticket').show();
                            } else {
                                $('#wk-bk-add-new-ticket').hide();
                                $('.wk-bk-data-row').each(
                                    function () {
                                        if ($(this).attr('data-repeat-index') != 0) {
                                            $(this).find('.action-delete').trigger('click');
                                        }
                                    }
                                );
                            }
                ...
                checkDefaultState: function () {
                    ...
                    initialize,
                    hasNotDefaultRecords = isRecordDataArray ? !!this.recordData().filter(
                        function (data) {
                    ...
                    if (!this.hasInitialPagesState[this.currentPage()] && isRecordDataArray && hasNotDefaultRecords) {
                        this.hasInitialPagesState[this.currentPage()] = true;
                        this.defaultPagesState[this.currentPage()] = utils.copy(
                            this.getChildItems().filter(
                                function (data) {
                ...
                sort: function (position, elem) {
                    ...
                    sorted = this.elems().sort(
                        function (propOne, propTwo) {
                ...
                setDifferedFromDefault: function () {
                    ...
                    Array.isArray(recordData) && recordData.forEach(
                        function (item) {
