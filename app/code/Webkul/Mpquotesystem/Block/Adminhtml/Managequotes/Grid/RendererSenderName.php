...
class RendererSenderName extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\AbstractRenderer
{
    /**
    ...
    public function render(\Magento\Framework\DataObject $row)
    {
        ...
        if ($row->getSender() == 0) {
            ...
            $senderData = $this->_mpquoteHelper->getCustomerData($row->getSender());
            if ($row->getSender()==$quote->getCustomerId()) {
                $actions[] = $senderData->getName()." ".__('(Customer)');
            } else {
                $actions[] = $senderData->getName()." ".__('(Seller)');
            }
        }
