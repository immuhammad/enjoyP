...
class Marketplace
{
    ...
    public function afterGetAllowedProductType(\Webkul\Marketplace\Helper\Data $subject, $result)
    {
        try {
            ...
            /**
             * For fixing the issue of booking products not assigning from customer section in admin panel
             */
            if (!empty($params['sellerassignproid'])) {
                $productIds = $params['sellerassignproid'];
                $productIds = $this->helper->getJsonDecodedString($productIds);
                foreach ($productIds as $key => $productId) {
                    $productType = $this->helper->getProduct($key)->getTypeId();
                    if (in_array($productType, $bookingProductTypes)
                        && !in_array($productType, $allowedTypes)
                    ) {
