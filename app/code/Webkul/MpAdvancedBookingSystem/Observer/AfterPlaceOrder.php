...
class AfterPlaceOrder implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        try {
            ...
            foreach ($orderedItems as $item) {
                ...
                $this->checkCancellationOption($item);
            }
     ...
     * @return null
     */
    protected function checkCancellationOption($item)
    {
        $product = $item->getProduct();
        if ($product->getCancellationAvailable()) {
            $item->setIsCancellationAvailable(1)
                ->save();
        }
    }
    ...
    public function setBookedSlotsInfo($item, $order)
    {
        try {
            ...
            if ($item->getProductType()=="hotelbooking") {
                ...
                $checkinTime = $item->getProduct()->getData('check_in_time');
                $checkoutTime = $item->getProduct()->getData('check_out_time');
                if (!($checkinTime || $checkoutTime)) {
                    $checkinTime = "11:00 am";
                    $checkoutTime = "11:00 am";
                }
                $bookingFrom = explode(' ', $selectedBookingDate)[0];
                $bookingTo = explode(' ', $selectedBookingDateTo)[0];
                $slotData = [
                    'booking_from' => $bookingFrom." ".$checkinTime,
                    'booking_to' => $bookingTo." ".$checkoutTime
                ];
            } else {
