...
class AfterProductSave implements ObserverInterface
{
    ...
    /**
     * @var \Webkul\MpAdvancedBookingSystem\Model\ResourceModel\SlotFactory
     */
    protected $_resSlotFactory;

    /**
     * @var \Webkul\MpAdvancedBookingSystem\Model\ResourceModel\InfoFactory
     */
    protected $_resInfoFactory;
    protected $logger;
    /**
     * Constructor
     *
     ...
     * @param \Webkul\MpAdvancedBookingSystem\Model\ResourceModel\SlotFactory $resSlotFactory
     * @param \Webkul\MpAdvancedBookingSystem\Model\ResourceModel\InfoFactory $resInfoFactory
    ...
    public function __construct(
        ...
        \Webkul\MpAdvancedBookingSystem\Model\ResourceModel\SlotFactory $resSlotFactory,
        \Webkul\MpAdvancedBookingSystem\Model\ResourceModel\InfoFactory $resInfoFactory,
        ...
        $this->_resSlotFactory = $resSlotFactory;
        $this->_resInfoFactory = $resInfoFactory;
            $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/custom.log');
            $this->logger = new \Zend_Log();
            $this->logger->addWriter($writer);
            $this->logger->info('text -- '.__FILE__." ".__LINE__);
    }
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        try {
            ...
            $this->logger->info('text -- '.$hotelAttrSetId);
    ...
    public function saveEventBooking($product, $data, $productId, $productSetId)
    {
        try {
            ...
            $slotProductId = 0;
            $slotCollection = $this->_slotCollection->create()
                            ->addFieldToFilter("product_id", $productId);
            foreach ($slotCollection as $value) {
                $slotProductId = $value->getId();
            }
            ...
            $slotModel->load($slotProductId)
            ->addData($slotData);
            $this->_resSlotFactory->create()->save($slotModel);
            $bookingProductId = 0;
            $bookingCollection = $this->_infoCollection->create()
                                ->addFieldToFilter("product_id", $productId);
            foreach ($bookingCollection as $value) {
                $bookingProductId = $value->getId();
            }
            ...
            $infoModel->load($bookingProductId)
            ->addData($infoData);
            $this->_resInfoFactory->create()->save($infoModel);
        } catch (\Exception $e) {
            $this->_bookingHelper->logDataInLogger($e->getMessage());
        }
    }
    ...
    public function saveRentalBooking($product, $data, $productId, $productSetId)
    {
        try {
            ...
            $slotCollection = $slotModel->getCollection()
                ->addFieldToFilter("product_id", $productId);
            if ($slotCollection->getSize()) {
                $bookingSlotId = $slotCollection->getFirstItem()->getId();
            }

            $bookingInfoId = 0;
            $infoModelOld = null;
            $infoBookingColl = $infoModel->getCollection()
                ->addFieldToFilter("product_id", $productId);
            if ($infoBookingColl->getSize()) {
                $infoModelOld = $infoBookingColl->getFirstItem();
                $bookingInfoId = $infoModelOld->getId();
            }
            ...
            if ($this->canSaveRentalSlots(
                ...
                $resSlot = $this->_resSlotFactory->create();
                if ($bookingSlotId) {
                    $slotModel->load($bookingSlotId);
                }
                $slotModel->addData($slotData);
                $resSlot->save($slotModel);

                if ($bookingInfoId) {
                    $infoModel->load($bookingInfoId);
                }
                $infoModel->addData($infoData);
                $resInfo = $this->_resInfoFactory->create();
                $resInfo->save($infoModel);
            }
     ...
     *
     * @param null|object $infoModelOld
     * @param string      $newJsonData
     * @param int         $type
    ...
    public function saveDefaultBooking($data, $productId)
    {
        try {
            ...
            if (!empty($data['sources']['assigned_sources'])) {
                $qty = 0;
                foreach ($data['sources']['assigned_sources'] as $key => $sourceData) {
                    if (!$sourceData['status']) {
                        continue;
                    }
                    if ($sourceData['source_code'] == 'default') {
                        $qty = $qty + $sourceData['quantity'];
                    }
                }
            }
