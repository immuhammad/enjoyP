...
/**
 * Webkul MpAdvancedBookingSystem MassStatusQuestion controller.
 */
class MassStatusQuestion extends Action implements \Magento\Framework\App\CsrfAwareActionInterface
{
    /**
     * @var Filter
     */
            ...
            try {
                ...
                if ($sellerProducts->getSize()) {
                    foreach ($collection as $questionData) {
                        $questionData->setStatus($params['status'])->save();
                    }
                }
                ...
                $this->messageManager->addSuccess(
                    __('%1 Records are succesfully updated.', $totalRecords)
                );
            } catch (\Exception $e) {
                $this->helper->logDataInLogger(
                    "Controller_Hotelbooking_Ui_MassStatusQuestion_execute Exception : ".$e->getMessage()
                );
